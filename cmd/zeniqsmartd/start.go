...
import (
	...
	"github.com/zeniqsmart/zeniq-smart-chain/rpc"
)
...
const (
	...
	flagBlocksBehind         = "blocks-behind"
	flagUnlock               = "unlock"
...
func StartCmd(ctx *Context, appCreator AppCreator) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx.Logger.Info("starting Smart Chain with Tendermint")
	...
	cmd.Flags().Int64(flagBlocksBehind, -1, "Delay syncing N blocks before head to keep a running backup of the DBs")
...
func startTmNode(nodeCfg *tmcfg.Config,
	...
	if !(tmos.FileExists(nodeCfg.PrivValidatorKeyFile()) && tmos.FileExists(nodeCfg.PrivValidatorStateFile())) {
		os.Remove(nodeCfg.PrivValidatorKeyFile())
		os.Remove(nodeCfg.PrivValidatorStateFile())
	}
